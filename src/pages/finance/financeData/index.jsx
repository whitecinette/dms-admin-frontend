...
function FinanceData() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backendUrl}/finance/main-labels?${query.toString()}`);
  ...
  const fetchPopupData = async (label, role) => {
    setLoading(true);
    try {
      const res = await fetch(`${backendUrl}/finance/details/${label}?role=${role}`);
      const json = await res.json();
      if (json.success) {
  ...
  const filteredPopupData = popupData.filter(entry =>
    Object.values(entry).some(val => val?.toString().toLowerCase().includes(popupSearch.toLowerCase()))
  );
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  <button className="action-btn orange" onClick={() => fetchPopupData(item.label, "main")}>View Details</button>
                  <button className="action-btn" onClick={() => fetchPopupData(item.label, "sub")}>View Working</button>
                  <button className="delete-btn" onClick={() => window.confirm("Delete this entry?")}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="popup">
            ...
            <div className="popup-content">
              <table>
                <thead>
                  <tr>
                    {popupData[0] && Object.keys(popupData[0]).map((key, i) => (
                      <th key={i}>{key}</th>
                    ))}
                  </tr>
                </thead>
